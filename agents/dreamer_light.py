...
class DreamerAgent:
    def __init__(self, state_size, action_size, layers_size=[450,450],deque_len=2000):
        ...
        self.elementary_range = 120#сколько кадров у нас на выходе
        self.curiosity = 0.15
        #self.curiosity = 0
        ...
        #dream длится 9 минут примерно
        self.count_trajectories = 50
        self.trajectory_seed_count=110
        self.trajectories_percentile=0.0
        #внимание. Надо больше одной траектории. Чтобы оценить не просто наилучшую траекторию, а несколько наилучших, которые начинаются с разных действий
        self.count_steps = 1#очень связано с self.elementary_range и self.planning_horison. self.elementary_range*self.count_steps должно быть больше self.planning_horison, и желательно не на 1-2, а раза в 2.
        
        #110*(110*5-400)*50
        self.hash_size = 11
        self.hash_curiosity = 0.1
        
        self.raw_buffer = None#это буфер сырых картинок, нужен для рисования траекторий
        self.batch_offset = 13
        self.reset()
    ...
    def build_model(self,type_mdl):
        if type_mdl=='sr':
            ...
            x = input_state
            x = TimeDistributed(Dense(140, activation='selu',
            ...
            # Bottleneck here!
            x = Dense(1650, name='bottleneck', activation='tanh')(x)
            # Start scaling back up
            # No frame stack for output
            cells_for_one_state = 84
    ...
    def generate_trajectories(self, count=10, steps=3,s=None, a=None, a_nxt=None, s_lst=None, a_lst=None,p_repeat_action=0):
        ...
        #p_repeat_action - вероятность сгенерить следующий экшн таким же, как предыдущий
    ...
    def update_target_model(self):
        self.train_model(epochs=3,sub_batch_size=6000,verbose=False, draw=True)
        self.dream()
    ...
    def get_batch(self, windowed_frames, windowed_actions, dones, batch_size, indices):
        while True:
            for i in indices:
                start = i * int(self.batch_offset)
                middle = start + int(0.5 * batch_size)
    ...
    def draw_trajectory(self,trajectory,buffer):
        #нарисовать сгенерированную траекторию
        Path('imgs').mkdir(parents=True, exist_ok=True)
        #для каждого эмбеддинга найти наиболее похожий в s и найти кадр, соответствующий этому эмбеддингу
        buflen = len(buffer)
        s = np.array(self.s)[-buflen:]
        imgs = []
        trajectory = list(trajectory)
        trajectory = 10*[trajectory[0]] + trajectory
        for s_cur in trajectory:
            errors = np.sum(np.abs(s - s_cur),axis=1)
            amin = np.argmin(errors)
            img = Image.fromarray(buffer[amin])
            imgs.append(img)
        name = str(int(np.random.rand()*100000))
        name = 'traj'+name+'.gif'
        imgs[0].save("./imgs/"+name, save_all=True, append_images=imgs[1:], duration=1/5, loop=0)
        print('draw_trajectory: success')
    ...
    def train_model(self,epochs=4,sub_batch_size=None,verbose=False,screen_curious=False,draw=False,diaposon=[-800,-1]): 
        ...
        if len(self.s) < self.train_start:
            return
        if epochs==4:
            #значит, запущен обычный train_model, который не в конце эпизода. Нафиг его.
            return
        print(f'train, epochs',epochs)
        s = np.array(self.s)
        a = np.array(self.a)
        r = np.array(self.r)
        hash_arr = self.model_rand.predict(s,batch_size=4096)
        self.model_sh.fit(s, hash_arr, batch_size=self.batch_size,
                           epochs=5, verbose=verbose)
        
        batch_size = self.elementary_range*2
        num_batches = ((len(self.s)-batch_size) // self.batch_offset)
        ...
        if draw and self.is_fit:
            ...
            sy_batches_predict = self.model_sas.predict(x,batch_size=2048)
            ...
            for i in range(trials):
                ...
                if idx!=-1:
                    ...
                    for j in range(steps):
                        ...
                        try:
                            sx_batch=self.model_sas.predict(x,batch_size=2048)
                        except Exception:
        ...
        prev_value = -1
        t = pd.Timestamp.now()
        max_size_train = 70#по-моему керас пытается все батчи отработать в один прогон, что приводит к OOM

        print(f'fitting sas... indices={len(indices)}')
        size_train = np.min([max_size_train,int((1-validation_split)*num_batches)])
        if size_train<2:
            size_train=num_batches
        self.model_sas.fit(x=self.get_batch(
            np.array(self.s), np.array(self.a),np.ravel(np.array(self.done)), 
            batch_size, indices[:size_train]),
            steps_per_epoch=size_train,
            epochs=np.max([int(epochs),1]),
            verbose=False)
        ...
        if draw:
            ...
            print('sr test rmse', np.sqrt(mse),'sr rmse/std', np.sqrt(mse)/np.std(r), 'sr rmse/mean', np.sqrt(mse)/np.mean(np.abs(r)), 's.shape', np.shape(s))
            if draw:
                #пересчитаем всё заново
                r_sr_predicted_local = self.model_sr.predict(np.array(self.s),batch_size=4096)
                print('sr fact, frc, test')
        ...
        if np.std(r)==0:
            ...
            a_lst = [a]
            r_lst = [r]
            aug_noise_ampl = 0.09*np.std(s)
            for i in range(aug_mult_count):
                s_lst.append(s+np.random.normal(size=np.shape(s))*aug_noise_ampl)
                a_lst.append(a)
                r_lst.append(r)
            s = np.concatenate(s_lst)
            a = np.concatenate(a_lst)
            r = np.concatenate(r_lst)
            
            self.model_sr.fit(s, r, batch_size=8200,epochs=epochs*10, verbose=False)
            mse = self.test_model(self.model_sr,s,np.array(np.ravel(r),ndmin=2,dtype=np.float32).T)
            print('sr train rmse', np.sqrt(mse),'sr rmse/std', np.sqrt(mse)/np.std(r), 'sr rmse/mean', np.sqrt(mse)/np.mean(np.abs(r)), 's.shape', np.shape(s), f'{pd.Timestamp.now()-t}')
            if draw:
                #пересчитаем всё заново
                r_sr_predicted_local = self.model_sr.predict(np.array(self.s),batch_size=4096)
                print('sr fact, frc, train')
                plt.plot(np.ravel(np.array(self.r)[diaposon[0]:diaposon[1]]))
                plt.plot(np.ravel(r_sr_predicted_local[diaposon[0]:diaposon[1]]))
                plt.show()
        self.is_fit = True
    ...
    def dream(self,diaposon=[-600,-1]):
        ...
        for i in range(self.trajectory_seed_count):
            ...
            if idx>0:
                ...
                s_nxt,a_nxt,a = self.generate_trajectories(count=self.count_trajectories, steps=self.count_steps,s=np.array(self.s)[idx:idx+self.elementary_range], a=np.array(self.a)[idx:idx+self.elementary_range], p_repeat_action=0.92)
                r=self.model_sr.predict(np.concatenate(s_nxt,axis=0), batch_size=4096)
                
                
                hash_arr = self.model_rand.predict(np.concatenate(s_nxt,axis=0), batch_size=4096)
                hash_arr_pred = self.model_sh.predict(np.concatenate(s_nxt,axis=0), batch_size=4096)
                hash_err = np.ravel(np.sum(np.abs(hash_arr-hash_arr_pred),axis=1)/np.std(hash_arr))
                r_curio = np.ravel(self.hash_curiosity*hash_err)
                r = np.ravel(r) + np.ravel(r_curio)
                ...
                s_lst.append(s_nxt[:-1])
                ns_lst.append(s_nxt[1:])
                a_lst.append(a_nxt[:-1])
                #лагало по памяти
                del hash_arr
                del hash_err
                del hash_arr_pred
        ...
        ns_lst=np.concatenate(ns_lst)
        #нарисовать траекторию, конечно же!
        if not (self.raw_buffer is None):
            self.draw_trajectory(s_lst[0,:],self.raw_buffer)
        a_lst = np.concatenate(a_lst,axis=0)
        #s_lst - тензор countXtime_lenXstate_size
        #a_lst - тензор countXtime_lenXa_size
        
        s=np.concatenate(s_lst)
        ns=np.concatenate(ns_lst)
        a=np.concatenate(a_lst)
        d_lst = np.zeros( (np.shape(s_lst)[0],np.shape(s_lst)[1]) )
        d_lst[:,-1] = 1
        d=np.ravel(d_lst)
        
        print(f'generated states raw:{np.shape(s)}, std(s[:,1]):{np.std(s[:,1])}, {pd.Timestamp.now()-t}')
        r=self.model_sr.predict(s, batch_size=4096)
        plt.title(f'rewards:{(np.min(r),np.median(r),np.max(r))}')
        plt.hist(np.ravel(r),bins=50)
        plt.show()
        hash_arr = self.model_rand.predict(s, batch_size=4096)
        hash_arr_pred = self.model_sh.predict(s, batch_size=4096)
        hash_err = np.sum(np.abs(hash_arr-hash_arr_pred),axis=1)/np.std(hash_arr)
        #print('hash_err',np.shape(hash_err),pd.Timestamp.now()-t)
        r_curio = hash_err*self.hash_curiosity
        #Рассчитать r_disco для всех траекторий
        r = r + np.reshape(r_curio,np.shape(r))
        
        t = pd.Timestamp.now()
        for step in range(self.q_steps):
            if step%10==0:
                print(f'q-step {step}/{self.q_steps}')
            if self.q_fit:
                q = r + self.discount_factor*self.model_sq.predict(ns,batch_size=4096)
            else:
                q = r
            self.q_fit = True
            #добалансировать
            mean = np.mean(q)
            mean75 = np.percentile(q,75)
            mean25 = np.percentile(q,25)
            if mean75==mean25:
        ...
        print('a,q,r, advantage',np.shape(a),np.shape(q),np.shape(r),np.shape(advantage))
        advantage += a*(q - r)
        print('advantage',advantage)
        self.model_sa.fit(s,advantage,epochs=epochs,
                    batch_size=8000,
                    verbose=False)
        print('model_sa fit')
        q_pred = self.model_sq.predict(np.array(self.s), batch_size=4096)
