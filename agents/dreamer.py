...
class DreamerAgent:
    def __init__(self, state_size, action_size, layers_size=[450,450],deque_len=2000):
        ...
        self.elementary_range = 130#сколько кадров у нас на выходе
        self.curiosity = 0.15
        #self.curiosity = 0
        self.layers_size = layers_size
        self.deque_len = deque_len
        self.count_trajectories = 80
    ...
    def train_model(self,epochs=4,sub_batch_size=None,verbose=False,screen_curious=False,draw=False,diaposon=[-700,-1]): 
    ...
    def dream(self,diaposon=[-700,-1]):
        ...
        for i in range(self.trajectory_seed_count):
            for j in range(200):
                #создать seed
                idx = int(np.random.rand()*(len(self.s)-1-self.elementary_range*(self.count_steps+2)))
                if np.sum(done[idx:idx+self.elementary_range*self.count_steps])==0:
                    break
                else:
                    idx=-1
            if idx>0:
                ...
                a_lst.append(a_nxt)
        if len(s_lst)==0:
            print('could not make seeds for trajectories')
            return
        ...
        #сделать таблицу sarn, где n - next state
        #s_lst: countXstate_sizeXtime_len. Contcat-0: 
        s = []
        ns = []
        a = []
        #тормозной способ
        #print(f's_lst:{np.shape(s_lst)}, a_lst:{np.shape(a_lst)}')
        for i in range(np.shape(s_lst)[1]-1):
            s.append(s_lst[:,i,:])
            a.append(a_lst[:,i,:])
            ns.append(s_lst[:,i+1,:])
        #print(f's:{np.shape(s)}, a:{np.shape(a)}')
        s=np.concatenate(s)
        ns=np.concatenate(ns)
        a=np.concatenate(a)
        print(f'generated states:{np.shape(s)[0]}, std(s[:,1]):{np.std(s[:,1])}')
        
        r=self.model_sr.predict(ns, batch_size=2048)
        for step in range(self.q_steps):
            if step%10==0:
                print(f'q-step {step}/{self.q_steps}')
            if self.q_fit:
                q = r + self.discount_factor*self.model_sq.predict(ns)
            else:
                q = r
            self.q_fit = True
            #добалансировать
        ...
        self.model_sa.fit(s,advantage,epochs=60,
                    batch_size=200,
                    verbose=False)
        print('model_sa fit')
