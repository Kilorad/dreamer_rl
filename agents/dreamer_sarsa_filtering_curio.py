...
class DreamerAgent:
    def __init__(self, state_size, action_size, layers_size=[450,450],deque_len=2000):
        ...
        self.count_trajectories = 2500
        self.trajectory_seed_count=40
        self.trajectories_percentile=0.995
        ...
        self.count_steps = 5#очень связано с self.elementary_range и self.planning_horison. self.elementary_range*self.count_steps должно быть больше self.planning_horison, и желательно не на 1-2, а раза в 2.
    ...
    def update_target_model(self):
        self.train_model(epochs=140,sub_batch_size=6000,verbose=False, draw=True)
        self.dream()
    ...
    def train_model(self,epochs=4,sub_batch_size=None,verbose=False,screen_curious=False,draw=False,diaposon=[-800,-1]): 
        ...
        prev_value = -1
        t = pd.Timestamp.now()
        max_size_train = 100#по-моему керас пытается все батчи отработать в один прогон, что приводит к OOM
        for i in range(6):
        ...
        if np.std(r)==0:
            ...
            self.model_sr.fit(s, r, batch_size=6096,epochs=epochs*2, verbose=False)
            mse = self.test_model(self.model_sr,s,np.array(np.ravel(r),ndmin=2,dtype=np.float32).T)
            print('sr train rmse', np.sqrt(mse),'sr rmse/std', np.sqrt(mse)/np.std(r), 'sr rmse/mean', np.sqrt(mse)/np.mean(np.abs(r)), 's.shape', np.shape(s))
            if draw:
                #пересчитаем всё заново
                r_sr_predicted_local = self.model_sr.predict(np.array(self.s))
    ...
    def dream(self,diaposon=[-600,-1]):
        ...
        r_disco = utils.exp_smooth(r+np.reshape(r_curio,np.shape(r)),self.discount_factor,self.planning_horison,d)
